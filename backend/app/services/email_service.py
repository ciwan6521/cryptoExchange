...
Supports async sending. Falls back to logging if SMTP is not configured.
"""
...
def send_email(to: str, subject: str, html_body: str) -> bool:
    ...
    if not cfg:
        logger.warning("SMTP not configured — logging email instead: to=%s subject=%s", to, subject)
...
def send_password_reset_email(to: str, token: str) -> bool:
    ...
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; background: #0a0b0c; color: #e5e7eb; padding: 40px 30px; border-radius: 12px;">
        ...
        <p style="color: #4b5563; font-size: 12px; text-align: center;">
            &copy; Crypto4Pro. All rights reserved.
        </p>
    </div>
    """
